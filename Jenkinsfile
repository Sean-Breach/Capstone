	...
	stage('Setup Global Parameters'){
		steps {
			...
			script {
				...
				ecrURI = sh(script: "/bin/bash -c 'aws ecr describe-repositories --output json | jq -r \'.repositories[] | select(.repositoryName == \"$ecrRepoName\").repositoryUri'", returnStdout: true)
			}
